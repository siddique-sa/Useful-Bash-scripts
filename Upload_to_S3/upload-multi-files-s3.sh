...
filePath="${1:-example-local-file.ext}`date +\%Y-\%m-\%d`/"
...
# Initialize defaults
for fileFromDirectory in $filePath*
do
    echo "File ${fileFromDirectory}"
    fileRemote="${fileFromDirectory}"
    fileLocal="${fileFromDirectory}"
